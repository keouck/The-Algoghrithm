...
for n in range(-2, 3):
  ...
  img = run_deep_dream_simple(img=img, steps=10, step_size=0.01)
...
def run():
    img = run_deep_dream_with_octaves(img=download(url), step_size=0.01)
    img = tf.image.resize(img, base_shape)
    img = tf.image.convert_image_dtype(img/255.0, dtype=tf.uint8)
    im = PIL.Image.fromarray(np.array(img))
    im.save("static/assets/apod.jpeg")
